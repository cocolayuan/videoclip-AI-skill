...
### 2026-01-27
- **重复型口误删除边界错误**：`一内向向的小朋友` 把两个"向"都删了
  - 错误：删除 (23.63-23.93) 包含了两个"向"
  - 正确：只删第二个"向" (23.81-23.93)
  - **教训**：重复型口误必须逐字符查时间戳，只删重复的那一个字

```python
# 正确做法：逐字符打印时间戳确认边界
for i, c in enumerate(chars):
    if 83 <= i <= 87:
        print(f"[{i}] {c['char']} @ {c['start']:.2f}-{c['end']:.2f}s")
# [83] 一 @ 23.21-23.43s
# [84] 内 @ 23.43-23.63s
# [85] 向 @ 23.63-23.81s  ← 保留
# [86] 向 @ 23.81-23.93s  ← 删除这个
# [87] 的 @ 23.93-24.11s
```
